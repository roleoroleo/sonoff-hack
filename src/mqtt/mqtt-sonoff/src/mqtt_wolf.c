...
{
    /* Check rollover */
    if (packet_id_last >= MAX_PACKET_ID) {
        packet_id_last = 0;
    }

    return ++packet_id_last;
}
...
int mqtt_connect()
{
    ...
    wolf_connect.lwt_msg = &lwt_msg;
    wolf_connect.enable_lwt = 1;

    /* Will */
    lwt_msg.qos        = mqtt_conf->qos;
    lwt_msg.retain     = mqtt_conf->retain_birth_will;
    memset(topic, '\0', sizeof(topic));
    snprintf(topic, sizeof(topic) - 1, "%s/%s", mqtt_conf->mqtt_prefix, mqtt_conf->topic_birth_will);
    lwt_msg.topic_name = (const char *) topic;
    lwt_msg.buffer = (unsigned char *) mqtt_conf->will_msg;
    lwt_msg.total_len  = strlen(mqtt_conf->will_msg);
...
static int mqtt_tls_cb(MqttClient* client)
{
    ...
    if (debug) fprintf(stderr, "MQTT TLS setup completed\n");
                           ...
                           byte msg_new, byte msg_done)
{
    ...
    if (debug) fprintf(stderr, "Received message for topic '%s'\n", msg->topic_name);
    ...
    if (len >= MAX_LINE_LENGTH) {
        fprintf(stderr, "Message topic exceeds buffer size\n");
        return -1;
    }
    ...
    if (strcasecmp("switch_on", param) == 0) {
        ...
        if ((strcasecmp("low", value) == 0) || (strcasecmp("25", value) == 0)) {
            sprintf(cmd_line, "%s -s low", IPC_CMD);
            system(cmd_line);
        } else if ((strcasecmp("medium", value) == 0) || (strcasecmp("50", value) == 0)) {
            sprintf(cmd_line, "%s -s medium", IPC_CMD);
            system(cmd_line);
        } else if ((strcasecmp("high", value) == 0) || (strcasecmp("75", value) == 0)) {
        ...
        if (strcasecmp("auto", value) == 0) {
            ...
            system(cmd_line);
        } else if ((strcasecmp("on", value) == 0) || (strcasecmp("yes", value) == 0)) {
