...
static const char *help_str =
        ...
        "       --set_preset   [value] Set process to call for PTZ set preset\n"
...
static const struct option long_opts[] =
{
    ...
    { "set_preset",    required_argument, NULL, LongOpts::set_preset   },
...
void processing_cmd(int argc, char *argv[])
{
    ...
    while( (opt = getopt_long(argc, argv, short_opts, long_opts, NULL)) != -1 )
    {
        switch( opt )
        {
            ...
            case LongOpts::set_preset:
                        if( !service_ctx.get_ptz_node()->set_set_preset(optarg) )
                            daemon_error_exit("Can't set process for set preset: %s\n", service_ctx.get_ptz_node()->get_cstr_err());
