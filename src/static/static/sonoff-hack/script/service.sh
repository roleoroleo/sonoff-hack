...
start_onvif()
{
    ...
    if [[ $PTZ_PRESENT -eq 1 ]]; then
        ...
        echo "goto_home_position=/mnt/mmc/sonoff-hack/bin/ptz -f /mnt/mmc/sonoff-hack/etc/ptz_presets.conf -a go_preset -n 0 > /dev/null" >> $ONVIF_SRVD_CONF
        echo "set_preset=/mnt/mmc/sonoff-hack/bin/ptz -f /mnt/mmc/sonoff-hack/etc/ptz_presets.conf -a set_preset -n %s -e %s" >> $ONVIF_SRVD_CONF
    ...
    $SONOFF_HACK_PREFIX/bin/onvif_notify_server --conf_file $ONVIF_SRVD_CONF
}
...
start_wsdd()
{
    $SONOFF_HACK_PREFIX/bin/wsd_simple_server --pid_file /var/run/wsd_simple_server.pid --if_name $ONVIF_NETIF --xaddr "http://%s$D_HTTPD_PORT/onvif/device_service" -m `hostname` -n Sonoff
}
